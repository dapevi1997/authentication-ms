...
class FlywayConfigTest {
    ...
    @Test
    @DisplayName("Debe construir URL JDBC correctamente")
    void debeConstruirUrlJdbcCorrectamente() {
        // Arrange
        var properties = mock(MySqlConnectionProperties.class);
        when(properties.host()).thenReturn("testhost");
        when(properties.port()).thenReturn(3307);
        when(properties.database()).thenReturn("production_db");
        when(properties.username()).thenReturn("prod_user");
        when(properties.password()).thenReturn("prod_pass");
        ...
        try (MockedStatic<Flyway> flywayStatic = mockStatic(Flyway.class, CALLS_REAL_METHODS)) {
            ...
            String expectedUrl =
                    "jdbc:mysql://testhost:3307/production_db?createDatabaseIfNotExist=true";
            verify(configSpy).dataSource(expectedUrl, "prod_user", "prod_pass");
            verify(configSpy).locations("classpath:db/migration");
            verify(configSpy).baselineOnMigrate(true);
            verify(configSpy).baselineVersion("0");
            verify(configSpy).schemas("production_db");
        }
    }
}
