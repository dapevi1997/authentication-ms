...
public class RegisterUserUseCase {
    ...
    public Mono<User> registerUser(User user) {
        return userRepository.existByEmail(user.getEmail().getEmailUser())
                .flatMap(exits -> {
                    if (exits){
                        logger.warning("El usuario con email " + user.getEmail().getEmailUser() + " existe.");
                        return Mono.error(new DomainException("Email existe"));
                    }
                    return userRepository.save(user);
                })
                .doOnSuccess(savedUser -> logger.info("Usuario registrado exitosamente con id " + savedUser.getIdUser().toString()))
                .doOnError(error -> logger.severe("Error en registro de usuario. Mensaje: " + error.getMessage()));
    }
}
